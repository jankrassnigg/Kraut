...
static Kraut::BoundingBox GetBranchBBox(aeInt32 iBranch)
{
  const aeUInt32 uiType = g_Tree.m_TreeStructure.m_BranchStructures[iBranch].m_Type;
  const Kraut::SpawnNodeDesc& bnd = g_Tree.m_Descriptor.m_StructureDesc.m_BranchTypes[uiType];

  const Kraut::BoundingBox bb1 = g_Tree.m_TreeMesh[aeLod::Lod0].m_BranchMeshes[iBranch].m_BoundingBox[Kraut::BranchGeometryType::Branch];
  const Kraut::BoundingBox bb2 = g_Tree.m_TreeMesh[aeLod::Lod0].m_BranchMeshes[iBranch].m_BoundingBox[Kraut::BranchGeometryType::Frond];
  const Kraut::BoundingBox bb3 = g_Tree.m_TreeMesh[aeLod::Lod0].m_BranchMeshes[iBranch].m_BoundingBox[Kraut::BranchGeometryType::Leaf];

  Kraut::BoundingBox bf;
  bf.SetInvalid();
  if (!bb1.IsInvalid())
  {
...
static aeInt32 FindLeastProjectionAxis(aeInt32 iBranch, const Kraut::BoundingBox& bf)
{
...
static aeInt32 FindBranchOfType(aeUInt32 uiBranchType)
{
  ...
  for (aeUInt32 b = 0; b < g_Tree.m_TreeStructure.m_BranchStructures.size(); ++b)
  {
    if (g_Tree.m_TreeStructure.m_BranchStructures[b].m_Type != uiBranchType)
      continue;

    aeUInt32 uiScore = 0;
    for (aeUInt32 mt = 0; mt < Kraut::BranchGeometryType::ENUM_COUNT; ++mt)
      uiScore += g_Tree.m_TreeMesh[aeLod::Lod0].m_BranchMeshes[b].m_Mesh[mt].GetNumTriangles();
...
{
  g_Tree.CreateTreeRenderData(aeLod::Lod0);
  ...
  const Kraut::BoundingBox bf = GetBranchBBox(iBranch);
