...
{
  while ((desc.m_LodData[lod].m_Mode == aeLodMode::Disabled) || aeLodMode::IsImpostorMode (desc.m_LodData[lod].m_Mode))
...
static void AddQuad (aeSubMesh &SubMesh, GLIM_BATCH& gb, const aeVec3 vPos, float fHalfWidth, float fHeight, float fRotation, float fTexCoordU, float fTexWidth)
{
...
static void AddBillboard (aeSubMesh &SubMesh, GLIM_BATCH& gb, const aeVec3 vPos, float fHalfWidth, float fHeight, float fTexCoordU, float fTexWidth)
{
  aeMeshVertex vtx;
...
void aeTree::RenderTreeImpostor (aeLod::Enum lod)
{
  ...
  aeShaderManager::BindTexture ("tex_Diffuse", m_hLodTextures, aeMeshType::ImpostorTextureSampler);
  aeShaderManager::BindTexture ("tex_Normal", m_hLodTexturesN, aeMeshType::ImpostorTextureSampler);
  aeRenderAPI::setFaceCulling (AE_CULL_FACE_NONE);

  if (m_Descriptor.m_LodData[lod].m_Mode == aeLodMode::Billboard)
