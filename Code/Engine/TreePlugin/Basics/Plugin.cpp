  ...
  case aeEditorPluginEvent::MouseMove:
    {
      ...
      //if (pEvent->m_Data.m_MouseEvent->modifiers () & Qt::ControlModifier)
      {
        aeUInt32 x = pEvent->m_Data.m_MouseEvent->x ();
        aeUInt32 y = pEvent->m_Data.m_MouseEvent->y ();
        bool bCTRL = (pEvent->m_Data.m_MouseEvent->modifiers () & Qt::ControlModifier);
  ...
  case aeEditorPluginEvent::MousePress:
    {
          ...
          (pEvent->m_Data.m_MouseEvent->modifiers () & Qt::ShiftModifier))
      {
        ...
        if (!pPlugin->m_vPickedPosition.IsZeroVector ())
        {
          ...
          {
            aeBranch* pBranch = (aeBranch*) aeEditorPlugin::GetPickableObject (id).m_pObject;
        ...
        if (pPlugin->m_bLeftMouseDown)
        {
          ...
          if (id != 0)
          {
            ...
            if (pEvent->m_Data.m_MouseEvent->modifiers () & Qt::ControlModifier)
            {
              ...
              if (aeStringFunctions::CompareEqual (po.m_szType, "aeForce"))
              {
                ...
                aePhysicsObject::SetSelected (NULL);
...
AE_ON_GLOBAL_EVENT (aeTreeEdit_BranchDeleted)
{
  ...
  if (aeTreePlugin::GetSelectedBranchID () >= (aeInt32) g_Tree.m_Branches.size ())
  {
    aeTreePlugin::SetSelectedBranch (-1, 0);
    return;
  }

  if ((void*) (&g_Tree.m_Branches[aeTreePlugin::GetSelectedBranchID ()]) == param1.Ptr)
    aeTreePlugin::SetSelectedBranch (-1, 0);
}
...
void aeTreePlugin::SetSelectedBranch (aeInt32 iSelectedBranch, aeUInt32 uiBranchNode)
{
  ...
  {
    aeBranch* pSelected = &g_Tree.m_Branches[iSelectedBranch];
...
bool aeTreePlugin::LoadTree (const char* szFile)
{
  ...
  for (aeInt32 i = 0; i < aeBranchType::ENUM_COUNT; ++i)
  {
    if (g_Tree.m_Descriptor.m_BranchTypes[i].m_bUsed)
    {
      qtTreeEditWidget::s_pWidget->SetCurrentlyEditedBranchType ((aeBranchType::Enum) i, false);
      goto found;
    }
  }

  qtTreeEditWidget::s_pWidget->SetCurrentlyEditedBranchType (aeBranchType::Trunk1, false);
