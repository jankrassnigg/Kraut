...
static void EngineStartup(void)
{
  ...
  aeBranchGeometryType::TextureSampler[Kraut::BranchGeometryType::Branch] = aeTextureManager::getTextureSamplerID("BRANCH");

  ss.m_TextureFilter = AE_FILTER_ANISOTROPIC_8X;
  ss.m_TextureWrapU = AE_WRAP_REPEAT;
  ss.m_TextureWrapV = AE_WRAP_REPEAT;
  aeTextureManager::setTextureSampler("FROND", ss);
  aeBranchGeometryType::TextureSampler[Kraut::BranchGeometryType::Frond] = aeTextureManager::getTextureSamplerID("FROND");

  ss.m_TextureFilter = AE_FILTER_ANISOTROPIC_8X;
  ss.m_TextureWrapU = AE_WRAP_CLAMP;
  ss.m_TextureWrapV = AE_WRAP_CLAMP;
  aeTextureManager::setTextureSampler("LEAF", ss);
  aeBranchGeometryType::TextureSampler[Kraut::BranchGeometryType::Leaf] = aeTextureManager::getTextureSamplerID("LEAF");

  ss.m_TextureFilter = AE_FILTER_ANISOTROPIC_8X;
  ss.m_TextureWrapU = AE_WRAP_CLAMP;
  ss.m_TextureWrapV = AE_WRAP_CLAMP;
  aeTextureManager::setTextureSampler("IMPOSTOR", ss);
  aeBranchGeometryType::ImpostorTextureSampler = aeTextureManager::getTextureSamplerID("IMPOSTOR");
...
{
  AE_CHECK_ALWAYS(QApplication::instance() != nullptr, "The QApplication is not initialized.");

  QMainWindow* pMainWindow = nullptr;
  aeVariableRegistry::RetrieveRaw("system/qt/mainwidget", &pMainWindow, sizeof(QMainWindow*));

  AE_CHECK_ALWAYS(pMainWindow != nullptr, "Plugin requires a pointer to the Main Widget.");
