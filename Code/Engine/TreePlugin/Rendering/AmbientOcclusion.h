...
public:
  ...
  {
    aeDirection () {} 
    aeDirection (float f) { for (int i = 0; i < 6; ++i) m_fDir[i] = f; }
  ...
  static void Update (aeBBox box, float fCellSize);
  static void Clear (void) { m_Cells.clear (); m_uiCells[0] = 0; m_uiCells[1] = 0; m_uiCells[2] = 0; }
...
private:
  static inline aeUInt32 GetCellIndex (aeUInt32 x, aeUInt32 y, aeUInt32 z) { const aeUInt32 uiIndex = (z * m_uiCells[1] + y) * m_uiCells[0] + x; return uiIndex; }
