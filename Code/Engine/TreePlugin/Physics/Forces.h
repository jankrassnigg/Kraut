...
public:
  ...
  {
    m_fMinRadius = f;
    m_fMaxRadius = aeMath::Max(m_fMaxRadius, f);
    HasChanged();
  }
  void SetMaxRadius(float f)
  {
    m_fMaxRadius = f;
    m_fMinRadius = aeMath::Min(m_fMinRadius, f);
    HasChanged();
  }
  ...
  void SetBranchInfluences(aeUInt32 uiInfluences)
  {
    m_InfluencesBranchTypes.SetFlags(uiInfluences);
    AE_BROADCAST_EVENT(aeTreeEdit_TreeModified);
  }

  aeUInt32 GetBranchInfluences() const
  {
    return m_InfluencesBranchTypes.GetData();
  }

  const aeMatrix& GetTransform() const { return m_Gizmo.GetTransform(); }
