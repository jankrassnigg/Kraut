...
qtTreeConfigWidget* qtTreeConfigWidget::s_pWidget = NULL;
static bool g_bUpdateThumbnails = true;
static float g_fLastThumbnailUpdate = -10;
aeUInt32 g_uiThumbnailSize = 128;

qtTreeConfigWidget::qtTreeConfigWidget (QWidget* parent) : QDockWidget (parent)
{
  s_pWidget = this;
  ...
  aeMemory::FillWithZeros(&m_pSpheres[0], sizeof (QGraphicsRectItem*) * aeBranchType::ENUM_COUNT);
}
...
aeArray<aeUInt8> Thumbnail[aeBranchType::ENUM_COUNT];
QPixmap* g_pPixmap[aeBranchType::ENUM_COUNT] = { NULL };
...
void qtTreeConfigWidget::UpdateDisplay (void)
{
  ...
  if (g_bUpdateThumbnails && (aeTime::GetAppTime () - g_fLastThumbnailUpdate > 2.0f))
  {
    ...
    {
      if (!g_Tree.m_Descriptor.m_BranchTypes[bt].m_bUsed)
        continue;
  ...
  AE_CHECK_DEV ((iEditedType >= 0) && (iEditedType < aeBranchType::ENUM_COUNT), "Incorrect Branch Type %d", iEditedType);

  actionToggleVisibility->setChecked (g_Tree.m_Descriptor.m_BranchTypes[iEditedType].m_bVisible);

  aeNodePos NodePositions[aeBranchType::ENUM_COUNT];

  for (aeUInt32 i = 0; i < aeBranchType::ENUM_COUNT; ++i)
  {
    m_pSpheres[i] = NULL;

    NodePositions[i].bUsed = g_Tree.m_Descriptor.m_BranchTypes[i].m_bUsed;

    if (!g_Tree.m_Descriptor.m_BranchTypes[i].m_bUsed)
      continue;

    const bool bVisible = g_Tree.m_Descriptor.m_BranchTypes[i].m_bVisible;

    aeInt32 iGroup = (i / 3);

    NodePositions[i].iPosY = (aeBranchType::ENUM_COUNT / 3) * iSize - iGroup * iSize - iSize + iGroup * (iSize/4);
    ...
    QGraphicsTextItem* pText = m_Scene.addText (aeBranchType::EnumNames[i], QFont ("Arial", 8, 1));
  ...
  for (aeUInt32 i = 3; i < aeBranchType::ENUM_COUNT; ++i)
  {
    ...
    for (aeInt32 iLower = iRoundDown; iLower < iRoundDown + 3; ++iLower)
    {
      ...
      if (g_Tree.m_Descriptor.m_BranchTypes[iLower].m_bAllowSubType[i % 3])
      {
  ...
  const aeInt32 uiGroups = aeBranchType::ENUM_COUNT / 3;
...
void qtTreeConfigWidget::on_actionToggleVisibility_triggered ()
{
  ...
  g_Tree.m_Descriptor.m_BranchTypes[iEditedType].m_bVisible = !g_Tree.m_Descriptor.m_BranchTypes[iEditedType].m_bVisible;
...
void qtTreeConfigWidget::on_actionRemoveTreeComponent_triggered ()
{
  ...
  aeSpawnNodeDesc& Type = g_Tree.m_Descriptor.m_BranchTypes[iEditedType];
  ...
  for (aeUInt32 t = 0; t < aeBranchType::ENUM_COUNT; ++t)
  {
    if (g_Tree.m_Descriptor.m_BranchTypes[t].m_bUsed)
    {
      qtTreeEditWidget::s_pWidget->SetCurrentlyEditedBranchType ((aeBranchType::Enum) t, false);
      break;
    }
  }
...
static bool HasChild (aeInt32 iBranchType, aeInt32 iChild)
{
  ...
  return (iChildID < aeBranchType::ENUM_COUNT) && g_Tree.m_Descriptor.m_BranchTypes[iChildID].m_bUsed;
}
...
static bool HasParent (aeInt32 iBranchType, aeInt32 iParent)
{
  ...
  return (iParentID >= 0 && g_Tree.m_Descriptor.m_BranchTypes[iParentID].m_bUsed);
}

static bool HasConnectionToChild (aeInt32 iBranchType, aeInt32 iChild)
{
  aeInt32 iChildID = GetChildID (iBranchType, iChild);
  return (iChildID < aeBranchType::ENUM_COUNT) && (g_Tree.m_Descriptor.m_BranchTypes[iBranchType].m_bAllowSubType[iChild] && g_Tree.m_Descriptor.m_BranchTypes[iChildID].m_bUsed);
}
...
static bool HasConnectionToParent (aeInt32 iBranchType, aeInt32 iParent)
{
  ...
  aeInt32 iParentID = GetParentID (iBranchType, iParent);
  return (iParentID >= 0) && (g_Tree.m_Descriptor.m_BranchTypes[iParentID].m_bAllowSubType[iChild] && g_Tree.m_Descriptor.m_BranchTypes[iParentID].m_bUsed);
}
...
void qtTreeConfigWidget::UpdateHighlights (bool bClick, QMouseEvent* event)
{
  ...
  qtTreeEditWidget::s_pWidget->SetCurrentlyEditedBranchType ((aeBranchType::Enum) iEditedType, false);
  ...
  if ((event) && (event->button () == Qt::RightButton) && bClick)
  {
    ...
    if (bUnderAny)
    {
      ...
      if (iEditedType < aeBranchType::ENUM_COUNT - 3)
...

      {
        ...
        for (aeInt32 c = 0; c < 3; ++c)
        {
          if (HasChild (iEditedType, c))
          {
            ...
            {
              s.Format ("Remove Connection to '%s'", aeBranchType::EnumNames[GetChildID (iEditedType, c)]);
              pActions[c]->setIcon (QIcon (":/Icons/Icons/Delete.png"));
            }
            else
            {
              s.Format ("Add Connection to '%s'", aeBranchType::EnumNames[GetChildID (iEditedType, c)]);
...

      {
        ...
        for (aeInt32 c = 0; c < 3; ++c)
        {
          if (HasParent (iEditedType, c))
          {
            ...
            {
              s.Format ("Remove Connection to '%s'", aeBranchType::EnumNames[GetParentID (iEditedType, c)]);
              pActions[c]->setIcon (QIcon (":/Icons/Icons/Delete.png"));
            }
            else
            {
              s.Format ("Add Connection to '%s'", aeBranchType::EnumNames[GetParentID (iEditedType, c)]);
              pActions[c]->setIcon (QIcon (":/Icons/Icons/Add.png"));
            }
...
AE_ON_GLOBAL_EVENT (aeTreePlugin_SelectedBranchTypeChanged)
{
  ...
  {
    aeInt32 iEditedType = qtTreeEditWidget::s_pWidget->GetCurrentlyEditedBranchType ();
    qtTreeConfigWidget::s_pWidget->actionToggleVisibility->setChecked (g_Tree.m_Descriptor.m_BranchTypes[iEditedType].m_bVisible);
...
void qtTreeConfigWidget::on_actionSaveTreeComponent_triggered ()
{
  ...
  aeFileSystem::MakeValidPath (sPrevPreset.c_str (), true, &sAbsolutePath, NULL);
  ...
  g_Tree.m_Descriptor.m_BranchTypes[iEditedType].Save (File);
}

static bool LoadBranchType (aeInt32 iBranchType, QWidget* pParent)
{
  aeFileSystem::MakeValidPath ("BranchTypes/", false, &sPrevPreset, NULL);
  ...
  g_Tree.m_Descriptor.m_BranchTypes[iBranchType].Load (File);

  g_Tree.m_Descriptor.m_BranchTypes[iBranchType].m_bUsed = true;
  g_Tree.m_Descriptor.m_BranchTypes[iBranchType].m_bAllowSubType[0] = false;
  g_Tree.m_Descriptor.m_BranchTypes[iBranchType].m_bAllowSubType[1] = false;
  g_Tree.m_Descriptor.m_BranchTypes[iBranchType].m_bAllowSubType[2] = false;
...
void qtTreeConfigWidget::on_actionAddTreeComponent_triggered ()
{
  ...
  if (!g_Tree.m_Descriptor.m_BranchTypes[iEditedType].m_bUsed)
  {
  ...
  if (iNextType >= aeBranchType::ENUM_COUNT)
  {
  ...
  {
    if (!g_Tree.m_Descriptor.m_BranchTypes[iNext].m_bUsed)
      goto found;
  }
...
found:
  ...
  for (aeInt32 i = 0; i < 3; ++i)
    g_Tree.m_Descriptor.m_BranchTypes[(iEditedType / 3) * 3 + i].m_bAllowSubType[iSubType] = false; 

  g_Tree.m_Descriptor.m_BranchTypes[iEditedType].m_bAllowSubType[iSubType] = true;

  g_Tree.m_Descriptor.m_BranchTypes[iNext].Reset ();

  g_Tree.m_Descriptor.m_BranchTypes[iNext].m_bUsed = true;
  g_Tree.m_Descriptor.m_BranchTypes[iNext].m_bAllowSubType[0] = false;
  g_Tree.m_Descriptor.m_BranchTypes[iNext].m_bAllowSubType[1] = false;
  g_Tree.m_Descriptor.m_BranchTypes[iNext].m_bAllowSubType[2] = false;

  qtTreeEditWidget::s_pWidget->SetCurrentlyEditedBranchType ((aeBranchType::Enum) iNext, false);
...
void qtTreeConfigWidget::on_actionToggleUpConnection1_triggered ()
{
  ...
  g_Tree.m_Descriptor.m_BranchTypes[iEditedType].m_bAllowSubType[0] = !g_Tree.m_Descriptor.m_BranchTypes[iEditedType].m_bAllowSubType[0];
...
void qtTreeConfigWidget::on_actionToggleUpConnection2_triggered ()
{
  ...
  g_Tree.m_Descriptor.m_BranchTypes[iEditedType].m_bAllowSubType[1] = !g_Tree.m_Descriptor.m_BranchTypes[iEditedType].m_bAllowSubType[1];
...
void qtTreeConfigWidget::on_actionToggleUpConnection3_triggered ()
{
  ...
  g_Tree.m_Descriptor.m_BranchTypes[iEditedType].m_bAllowSubType[2] = !g_Tree.m_Descriptor.m_BranchTypes[iEditedType].m_bAllowSubType[2];
...
void qtTreeConfigWidget::on_actionToggleDownConnection1_triggered ()
{
  ...
  g_Tree.m_Descriptor.m_BranchTypes[iParentID].m_bAllowSubType[iEditedType%3] = !g_Tree.m_Descriptor.m_BranchTypes[iParentID].m_bAllowSubType[iEditedType%3];
