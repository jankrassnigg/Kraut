...
{
  m_pCurNT->m_bEnable[aeMeshType::Branch] = check_EnableBranchMesh->isChecked ();
  widgetBranchContent->setVisible(m_pCurNT->m_bEnable[aeMeshType::Branch]);
...
{
  aeTreeMaterial* pPrevMaterial = aeTreeMaterialLibrary::GetOrAddMaterial (m_pCurNT->m_sTexture[aeMeshType::Branch].c_str ());

  if (!SelectTexture (sPrevBranchTexture, m_pCurNT->m_sTexture[aeMeshType::Branch], NULL))
    return;

  // this will alreay make sure the combo box contains the material
  aeTreeMaterial* pMaterial = aeTreeMaterialLibrary::GetOrAddMaterial (m_pCurNT->m_sTexture[aeMeshType::Branch].c_str ());

  ComboBranchTexture->setCurrentIndex (m_MapBarkTextureToCombo[m_pCurNT->m_sTexture[aeMeshType::Branch]]);
...
{
  m_pCurNT->m_bEnable[aeMeshType::Frond] = check_EnableFronds->isChecked ();
  widgetFrondContent->setVisible (m_pCurNT->m_bEnable[aeMeshType::Frond]);
...
{
  m_pCurNT->m_FrondUpOrientation = (aeLeafOrientation::Enum) index;
...
{
  aeTreeMaterial* pPrevMaterial = aeTreeMaterialLibrary::GetOrAddMaterial (m_pCurNT->m_sTexture[aeMeshType::Frond].c_str ());

  if (!SelectTexture (sPrevFrondTexture, m_pCurNT->m_sTexture[aeMeshType::Frond], NULL))
    return;

  // this will alreay make sure the combo box contains the material
  aeTreeMaterial* pMaterial = aeTreeMaterialLibrary::GetOrAddMaterial (m_pCurNT->m_sTexture[aeMeshType::Frond].c_str ());

  ComboFrondTexture->setCurrentIndex (m_MapFrondTextureToCombo[m_pCurNT->m_sTexture[aeMeshType::Frond]]);
...
{
  m_pCurNT->m_FrondContourMode = (aeSpawnNodeDesc::FrondContourMode) index;
  AE_BROADCAST_EVENT (aeTreeEdit_TreeModified);
}
...
void qtTreeEditWidget::on_button_FrondColor_clicked ()
{
  ...
  g_uiLastFrondColor[0] = m_pCurNT->m_uiVariationColor[aeMeshType::Frond][0];
  g_uiLastFrondColor[1] = m_pCurNT->m_uiVariationColor[aeMeshType::Frond][1];
  g_uiLastFrondColor[2] = m_pCurNT->m_uiVariationColor[aeMeshType::Frond][2];
  g_uiLastFrondColor[3] = m_pCurNT->m_uiVariationColor[aeMeshType::Frond][3];

  QColor col;
  col.setRgb (m_pCurNT->m_uiVariationColor[aeMeshType::Frond][0], m_pCurNT->m_uiVariationColor[aeMeshType::Frond][1], m_pCurNT->m_uiVariationColor[aeMeshType::Frond][2], m_pCurNT->m_uiVariationColor[aeMeshType::Frond][3]);
...
{
  m_pCurNT->m_uiVariationColor[aeMeshType::Frond][0] = chosenColor.red ();
  m_pCurNT->m_uiVariationColor[aeMeshType::Frond][1] = chosenColor.green ();
  m_pCurNT->m_uiVariationColor[aeMeshType::Frond][2] = chosenColor.blue ();
  m_pCurNT->m_uiVariationColor[aeMeshType::Frond][3] = chosenColor.alpha ();

  sliderFrondAlpha->setValue (m_pCurNT->m_uiVariationColor[aeMeshType::Frond][3]);
...
{
  m_pCurNT->m_uiVariationColor[aeMeshType::Frond][0] = g_uiLastFrondColor[0];
  m_pCurNT->m_uiVariationColor[aeMeshType::Frond][1] = g_uiLastFrondColor[1];
  m_pCurNT->m_uiVariationColor[aeMeshType::Frond][2] = g_uiLastFrondColor[2];
  m_pCurNT->m_uiVariationColor[aeMeshType::Frond][3] = g_uiLastFrondColor[3];
...
{
  if ((m_pCurNT->m_uiVariationColor[aeMeshType::Frond][0] != g_uiLastFrondColor[0]) ||
      (m_pCurNT->m_uiVariationColor[aeMeshType::Frond][1] != g_uiLastFrondColor[1]) ||
      (m_pCurNT->m_uiVariationColor[aeMeshType::Frond][2] != g_uiLastFrondColor[2]) ||
      (m_pCurNT->m_uiVariationColor[aeMeshType::Frond][3] != g_uiLastFrondColor[3]))
...
{
  m_pCurNT->m_uiVariationColor[aeMeshType::Frond][3] = i;
...
{
  m_pCurNT->m_bEnable[aeMeshType::Leaf] = CheckEnableLeaves->isChecked ();
  WidgetLeafContent->setVisible (m_pCurNT->m_bEnable[aeMeshType::Leaf]);
  AE_BROADCAST_EVENT (aeTreeEdit_TreeModified);
}
...
{
  aeTreeMaterial* pPrevMaterial = aeTreeMaterialLibrary::GetOrAddMaterial (m_pCurNT->m_sTexture[aeMeshType::Leaf].c_str ());

  if (!SelectTexture (sPrevLeafTexture, m_pCurNT->m_sTexture[aeMeshType::Leaf], NULL))
    return;

  // this will alreay make sure the combo box contains the material
  aeTreeMaterial* pMaterial = aeTreeMaterialLibrary::GetOrAddMaterial (m_pCurNT->m_sTexture[aeMeshType::Leaf].c_str ());

  ComboLeafTexture->setCurrentIndex (m_MapLeafTextureToCombo[m_pCurNT->m_sTexture[aeMeshType::Leaf]]);
...
void qtTreeEditWidget::on_ButtonLeafColor_clicked ()
{
  ...
  g_uiLastLeafColor[0] = m_pCurNT->m_uiVariationColor[aeMeshType::Leaf][0];
  g_uiLastLeafColor[1] = m_pCurNT->m_uiVariationColor[aeMeshType::Leaf][1];
  g_uiLastLeafColor[2] = m_pCurNT->m_uiVariationColor[aeMeshType::Leaf][2];
  g_uiLastLeafColor[3] = m_pCurNT->m_uiVariationColor[aeMeshType::Leaf][3];

  QColor col;
  col.setRgb (m_pCurNT->m_uiVariationColor[aeMeshType::Leaf][0], m_pCurNT->m_uiVariationColor[aeMeshType::Leaf][1], m_pCurNT->m_uiVariationColor[aeMeshType::Leaf][2], m_pCurNT->m_uiVariationColor[aeMeshType::Leaf][3]);
...
{
  m_pCurNT->m_uiVariationColor[aeMeshType::Leaf][0] = chosenColor.red ();
  m_pCurNT->m_uiVariationColor[aeMeshType::Leaf][1] = chosenColor.green ();
  m_pCurNT->m_uiVariationColor[aeMeshType::Leaf][2] = chosenColor.blue ();
  m_pCurNT->m_uiVariationColor[aeMeshType::Leaf][3] = chosenColor.alpha ();

  SliderLeafAlpha->setValue (m_pCurNT->m_uiVariationColor[aeMeshType::Leaf][3]);
...
{
  m_pCurNT->m_uiVariationColor[aeMeshType::Leaf][0] = g_uiLastLeafColor[0];
  m_pCurNT->m_uiVariationColor[aeMeshType::Leaf][1] = g_uiLastLeafColor[1];
  m_pCurNT->m_uiVariationColor[aeMeshType::Leaf][2] = g_uiLastLeafColor[2];
  m_pCurNT->m_uiVariationColor[aeMeshType::Leaf][3] = g_uiLastLeafColor[3];
...
{
  if ((m_pCurNT->m_uiVariationColor[aeMeshType::Leaf][0] != g_uiLastLeafColor[0]) ||
      (m_pCurNT->m_uiVariationColor[aeMeshType::Leaf][1] != g_uiLastLeafColor[1]) ||
      (m_pCurNT->m_uiVariationColor[aeMeshType::Leaf][2] != g_uiLastLeafColor[2]) ||
      (m_pCurNT->m_uiVariationColor[aeMeshType::Leaf][3] != g_uiLastLeafColor[3]))
    AE_BROADCAST_EVENT (aeTreeEdit_TreeModifiedSimple);
...
{
  m_pCurNT->m_uiVariationColor[aeMeshType::Leaf][3] = i;
...
void qtTreeEditWidget::on_ComboBranchTexture_currentIndexChanged (int index)
{
  ...
  {
    aeTreeMaterial* pPrevMaterial = aeTreeMaterialLibrary::GetOrAddMaterial (m_pCurNT->m_sTexture[aeMeshType::Branch].c_str ());
    m_pCurNT->m_sTexture[aeMeshType::Branch] = ComboBranchTexture->itemData (index).toString ().toUtf8 ().data ();
    aeTreeMaterial* pMaterial = aeTreeMaterialLibrary::GetOrAddMaterial (m_pCurNT->m_sTexture[aeMeshType::Branch].c_str ());
...
void qtTreeEditWidget::on_ComboFrondTexture_currentIndexChanged (int index)
{
  ...
  {
    aeTreeMaterial* pPrevMaterial = aeTreeMaterialLibrary::GetOrAddMaterial (m_pCurNT->m_sTexture[aeMeshType::Frond].c_str ());
    m_pCurNT->m_sTexture[aeMeshType::Frond] = ComboFrondTexture->itemData (index).toString ().toUtf8 ().data ();
    aeTreeMaterial* pMaterial = aeTreeMaterialLibrary::GetOrAddMaterial (m_pCurNT->m_sTexture[aeMeshType::Frond].c_str ());
...
void qtTreeEditWidget::on_ComboLeafTexture_currentIndexChanged (int index)
{
  ...
  {
    aeTreeMaterial* pPrevMaterial = aeTreeMaterialLibrary::GetOrAddMaterial (m_pCurNT->m_sTexture[aeMeshType::Leaf].c_str ());
    m_pCurNT->m_sTexture[aeMeshType::Leaf] = ComboLeafTexture->itemData (index).toString ().toUtf8 ().data ();
    aeTreeMaterial* pMaterial = aeTreeMaterialLibrary::GetOrAddMaterial (m_pCurNT->m_sTexture[aeMeshType::Leaf].c_str ());
