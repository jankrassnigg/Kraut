...
QToolButton* g_pRenderModeButton = NULL;
QToolButton* g_pOpenFileButton = NULL;
QToolButton* g_pTransformModeButton = NULL;
QMenu* g_pRecentFileMenu = NULL;

qtTreeEditWidget* qtTreeEditWidget::s_pWidget = NULL;
...
qtTreeEditWidget::qtTreeEditWidget (QWidget* parent) : QDockWidget (parent)
{
  s_pWidget = this;

  m_CurrentlyEditedType = aeBranchType::Trunk1;
  ...
  for (aeUInt32 i = 0; i < aeTargetDir::ENUM_COUNT; ++i)
    combo_BranchTargetDir->addItem (aeTargetDir::EnumNames[i]);
  for (aeUInt32 i = 0; i < aeTargetDir::ENUM_COUNT; ++i)
    combo_BranchTargetDir2->addItem (aeTargetDir::EnumNames[i]);
  for (aeUInt32 i = 0; i < aeLod::ENUM_COUNT; ++i)
    combo_LoD->addItem (aeLod::EnumNames[i]);
  for (aeUInt32 i = 0; i < aeLeafOrientation::ENUM_COUNT; ++i)
    combo_FrondOrientation->addItem (aeLeafOrientation::EnumNames[i]);
  ...
  for (aeUInt32 i = 0; i < aeBranchTypeMode::ENUM_COUNT; ++i)
    ComboBranchTypeMode->addItem (aeBranchTypeMode::EnumNames[i]);

  combo_LoD->setCurrentIndex (aeLod::Lod0);
  ComboBranchTypeMode->setCurrentIndex (aeBranchTypeMode::Default);
...
void qtTreeEditWidget::UpdateStats ()
{
  ...
  lcd_PolygonsBranch->display ((int) g_Tree.GetNumTriangles (g_Globals.s_CurLoD, aeMeshType::Branch));
  lcd_PolygonsFrond->display  ((int) g_Tree.GetNumTriangles (g_Globals.s_CurLoD, aeMeshType::Frond));
  lcd_PolygonsLeaves->display  ((int) g_Tree.GetNumTriangles (g_Globals.s_CurLoD, aeMeshType::Leaf));
}
...
AE_ON_GLOBAL_EVENT (aeTreePlugin_SelectedBranchChanged)
{
  if (param0.iInt32 != -1)
  {
    aeBranch* pBranch = &g_Tree.m_Branches[param0.iInt32];
...
void qtTreeEditWidget::UpdateMaterialBoxes (void)
{
  ...
  if (m_pCurNT)
  {
    aeTreeMaterialLibrary::GetOrAddMaterial (m_pCurNT->m_sTexture[aeMeshType::Branch].c_str ());
    aeTreeMaterialLibrary::GetOrAddMaterial (m_pCurNT->m_sTexture[aeMeshType::Frond].c_str ());
    aeTreeMaterialLibrary::GetOrAddMaterial (m_pCurNT->m_sTexture[aeMeshType::Leaf].c_str ());
  }
...
void qtTreeEditWidget::SetCurrentlyEditedBranchType (aeBranchType::Enum Type, bool bForce)
{
  AE_PREVENT_RECURSION
  ...
  m_pCurNT = &g_Tree.m_Descriptor.m_BranchTypes[m_CurrentlyEditedType];

  // General
  {
    spin_RandomSeed->setValue (g_Tree.m_Descriptor.m_uiRandomSeed);
  ...
  // Branch Type
  {
    ...
    //le_BranchTexture->setText (m_pCurNT->m_sTexture[aeMeshType::Branch].c_str ());
    //le_BranchTextureN->setText (m_pCurNT->m_sTextureN[aeMeshType::Branch].c_str ());
  ...
  // Leaf Cards
  {
    check_LeafCardMode->setChecked (g_Tree.m_Descriptor.m_bLeafCardMode);
  ...
  // Manual Editing
  {
    ...
    check_CreateProcedural->setChecked (g_Tree.m_Descriptor.m_bGrowProceduralTrunks);
  }
  ...
  // Fronds
  {
    ...
    check_EnableFronds->setChecked (m_pCurNT->m_bEnable[aeMeshType::Frond]);
    ...
    //le_FrondTexture->setText (m_pCurNT->m_sTexture[aeMeshType::Frond].c_str ());
    //le_FrondTextureN->setText (m_pCurNT->m_sTextureN[aeMeshType::Frond].c_str ());
  ...
  // Leaves
  {
    CheckEnableLeaves->setChecked (m_pCurNT->m_bEnable[aeMeshType::Leaf]);
    ...
    //EditLeafTexture->setText (m_pCurNT->m_sTexture[aeMeshType::Leaf].c_str ());
    //EditLeafTextureN->setText (m_pCurNT->m_sTextureN[aeMeshType::Leaf].c_str ());
  ...
  // Branch
  {
    check_EnableBranchMesh->setChecked (m_pCurNT->m_bEnable[aeMeshType::Branch]);
...
void qtTreeEditWidget::SetCurrentlyEditedLoD (aeLod::Enum LoD)
{
  ...
  for (aeUInt32 mt = 0; mt < aeMeshType::ENUM_COUNT; ++mt)
  {
    for (aeUInt32 bt = 0; bt < aeBranchType::ENUM_COUNT; ++bt)
    {
      ...
      cbLod[mt][bt]->setVisible (g_Tree.m_Descriptor.m_BranchTypes[bt].m_bUsed && g_Tree.m_Descriptor.m_BranchTypes[bt].m_bEnable[mt]);
    }
  }
...
bool qtTreeEditWidget::SelectFile (aeFilePath& sPrevFile, aeHybridString<128>& sTargetString, QLineEdit* pLineEdit, const char* szFileTypeFilter, const char* szContentSubFolder)
{
  // get the current files's full path
  {
    ...
    if (File.Open (sTargetString.c_str ()))
      ...
      aeFileSystem::MakeValidPath (szContentSubFolder, false, &sPrevFile, NULL);
  }
...
{
  const bool bEnableGUI = m_pCurNT->m_BranchTypeMode != aeBranchTypeMode::Umbrella;

  if (m_pCurNT->m_BranchTypeMode == aeBranchTypeMode::Umbrella)
    m_pCurNT->m_FrondContourMode = aeSpawnNodeDesc::Symetric;
