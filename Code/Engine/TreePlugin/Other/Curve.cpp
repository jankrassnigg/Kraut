...
void aeCurve::Initialize (aeUInt32 uiSamples, float fInitVal, float fMin, float fMax)
{
  m_fMinValue = fMin;
  m_fMaxValue = fMax;
...
float aeCurve::GetValueAt (float fPosition) const
{
...
float aeCurve::GetValueAtMirrored (float fPosition) const
{
  fPosition *= 2.0f;
...
void aeCurve::PasteCurve (const aeCurve& OtherCurve)
{
...
void aeCurve::Save (aeStreamOut& s)
{
...
void aeCurve::Load (aeStreamIn& s, bool bOldFormat)
{
