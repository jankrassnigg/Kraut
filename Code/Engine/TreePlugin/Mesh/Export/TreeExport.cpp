...
void aeTree::KrautExport_GatherMaterial(Kraut::BranchType::Enum bt, Kraut::BranchGeometryType::Enum mt, aeMap<aeKrautMaterial, aeSet<aeUInt32>>& Materials)
{
  const Kraut::SpawnNodeDesc& sn = m_Descriptor.m_StructureDesc.m_BranchTypes[bt];
...
void aeTree::KrautExport_GatherMaterials(aeMap<aeKrautMaterial, aeSet<aeUInt32>>* out_Materials)
{
  for (aeUInt32 bt = 0; bt < Kraut::BranchType::ENUM_COUNT; ++bt)
  {
    ...
    for (aeUInt32 mt = 0; mt < Kraut::BranchGeometryType::ENUM_COUNT; ++mt)
      KrautExport_GatherMaterial((Kraut::BranchType::Enum)bt, (Kraut::BranchGeometryType::Enum)mt, out_Materials[mt]);
  }
}
...
void aeTree::KrautExport_GatherSubMeshesOfType(aeLod::Enum lod, Kraut::BranchGeometryType::Enum mt, const aeSet<aeUInt32>& Types, aeDeque<const Kraut::Mesh*>& out_SubMeshes)
{
  // go through hall branches, search for branches that use the current material
  const aeUInt32 uiBranches = m_TreeStructure.m_BranchStructures.size();
  for (aeUInt32 b = 0; b < uiBranches; ++b)
  {
    // it the type of this branch uses the material that 'it' currently points to
    if (Types.find(m_TreeStructure.m_BranchStructures[b].m_Type) != Types.end())
    {
      // This sub-mesh should be added to the file
      const Kraut::Mesh& mesh = m_TreeMesh[lod].m_BranchMeshes[b].m_Mesh[mt];

      if (!mesh.m_Triangles.empty())
        out_SubMeshes.push_back(&mesh);
    }
  }
}

static void CombineSubMeshes(const aeDeque<const Kraut::Mesh*> SubMeshes, Kraut::Mesh& out_Combined)
{
...
static void WriteSubMesh(aeStreamOut& Stream, const Kraut::Mesh& SubMesh, float fScale)
{
  ...
  for (aeUInt32 v = 0; v < uiVertices; ++v)
  {
    const Kraut::Vertex& mv = SubMesh.m_Vertices[v];
  ...
  for (aeUInt32 t = 0; t < uiTriangles; ++t)
  {
    const Kraut::Triangle& mt = SubMesh.m_Triangles[t];
...
bool aeTree::ExportKraut(const char* szFile, bool bExportBranches, bool bExportFronds, bool bExportLeaves, bool bTrunkCaps, bool bBranchCaps, bool bLodNone, bool bLod0, bool bLod1, bool bLod2, bool bLod3, bool bLod4)
{
  ...
  aeMap<aeKrautMaterial, aeSet<aeUInt32>> Materials[Kraut::BranchGeometryType::ENUM_COUNT];
  ...
  for (int lod = 0; lod < aeLod::ENUM_COUNT; ++lod)
  {
    ...
    {
      aeUInt8 uiMaterialTypes = Kraut::BranchGeometryType::ENUM_COUNT;
      ...
      for (aeUInt8 mt = 0; mt < uiMeshTypes; ++mt)
      {
        ...
        for (; it != itend; ++it)
        {
          aeDeque<const Kraut::Mesh*> SubMeshes;
          KrautExport_GatherSubMeshesOfType((aeLod::Enum)lod, (Kraut::BranchGeometryType::Enum)mt, it.value(), SubMeshes);

          // now we know ALL the sub-meshes in this lod, of this mesh-type, that use the same texture/material
          if (!SubMeshes.empty())
          {
        ...
        for (aeUInt32 m = 0; m < uiMeshes; ++m)
        {
          ...
          WriteSubMesh(Stream, Meshes[m].m_Mesh, fScale);
        }
      }
    }
    else if ((uiLodMode == 1) || (uiLodMode == 2) || (uiLodMode == 3))
    {
      const aeUInt8 uiMeshTypes = 1;
      Stream << uiMeshTypes;

      aeUInt8 mt = Kraut::BranchGeometryType::Frond;
      Stream << mt;
