...
#include <KrautGraphics/DynamicTree/Declarations.h>
#include <map>
...
namespace AE_NS_FOUNDATION
{
  ...
  {
  public:
    AE_GRAPHICS_DLL aeDynamicOctree();

    AE_GRAPHICS_DLL void CreateTree(const aeVec3& vCenter, const aeVec3& vHalfExtents, float fMinNodeSize);
    ...
    AE_GRAPHICS_DLL bool InsertObject(const aeVec3& vCenter, const aeVec3& vHalfExtents, aeInt32 iObjectType, aeInt32 iObjectInstance, bool bOnlyIfInside = false);
    //! Adds and object at position vCenter with bounding-box dimensions vHalfExtents to the tree. If the object is outside the tree and bOnlyIfInside is true, nothing will be inserted.
    AE_GRAPHICS_DLL bool InsertObject(const aeVec3& vCenter, const aeVec3& vHalfExtents, aeInt32 iObjectType, aeInt32 iObjectInstance, bool bOnlyIfInside, aeDynamicTreeObject& out_Object);
    ...
    //AE_GRAPHICS_DLL void FindVisibleObjects (const aeViewfrustum& Viewfrustum, AE_VISIBLE_OBJ_CALLBACK Callback, void* pPassThrough) const;

    //! Returns all objects that are in the same cell as the given point.
    AE_GRAPHICS_DLL void FindObjectsInRange(const aeVec3& vPoint, AE_VISIBLE_OBJ_CALLBACK Callback, void* pPassThrough) const;

    //! Returns all objects that are in the same area of the given sphere.
    AE_GRAPHICS_DLL void FindObjectsInRange(const aeVec3& vPoint, float fRadius, AE_VISIBLE_OBJ_CALLBACK Callback, void* pPassThrough) const;

    //! Removes the given Object. This is an O(1) operation.
    AE_GRAPHICS_DLL void RemoveObject(aeDynamicTreeObject obj);

    //! Removes the Instance of the given Type from the tree. SLOW: O(n)
    AE_GRAPHICS_DLL void RemoveObject(aeInt32 iObjectType, aeInt32 iObjectInstance);

    //! Removes ALL Instances of a given Type from the tree. O(n) time complexity.
    AE_GRAPHICS_DLL void RemoveObjectsOfType(aeInt32 iObjectType);

    //! Clears the tree from all objects.
    AE_GRAPHICS_DLL void RemoveAllObjects(void) { m_NodeMap.clear(); }

    //! Returns the trees BoundingBox.
    AE_GRAPHICS_DLL void getAABB(float& out_fMinX, float& out_fMaxX, float& out_fMinY, float& out_fMaxY, float& out_fMinZ, float& out_fMaxZ) const;
