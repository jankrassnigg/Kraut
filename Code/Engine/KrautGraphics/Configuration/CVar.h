...
#include <KrautGraphics/Utility/Enum.h>
...
{
  AE_ENUMERABLE_CLASS(AE_GRAPHICS_DLL, aeCVar);
  ...
  struct aeCVarFlags
  {
    enum Flags
    {
      ...
      DevOnly = 1 << 0,           //!< Only in Developer-Mode visible and writable (else not listed in Console)
  ...
  {
  public:
    AE_GRAPHICS_DLL aeCVar(const char* szName, aeUInt32 flags, aeCVarType::Enum type, const char* szDescription);

    //! Returns the name of this CVar.
    AE_GRAPHICS_DLL const char* GetName(void) const;
    //! Returns the descriptive text of this CVar.
    AE_GRAPHICS_DLL const char* GetDescription(void) const;
    //! Returns the type of this CVar.
    AE_GRAPHICS_DLL aeCVarType::Enum GetType(void) const;
    //! Returns all the flags of this CVar.
    AE_GRAPHICS_DLL const aeFlags32& GetFlags(void) const;

    //! Returns the CVar with the given name, or nullptr if that does not exist.
    AE_GRAPHICS_DLL static aeCVar* GetCVar(const char* szName);

    //! Registers functions in the given Lua-Script, such that it can directly read/write CVars.
    AE_GRAPHICS_DLL static void AllowScriptAccessToCVars(aeLuaWrapper& Script);

    //! Resets the CVar to its default value defined by the program.
    AE_GRAPHICS_DLL virtual void ResetToDefault(void) = 0;
    //! Resets the CVar to the value that is currently stored on file.
    AE_GRAPHICS_DLL virtual void ResetToStored(void) = 0;
    //! Sets the CVars current value to its restart value.
    AE_GRAPHICS_DLL virtual void SetToRestartValue(void) = 0;

    //! Writes all current values of the CVars to the given config-file.
    AE_GRAPHICS_DLL static bool SaveCVars(const char* szSaveToFile);
    //! Reads all current values of the CVars from the given config-file.
    AE_GRAPHICS_DLL static bool LoadCVars(const char* szLoadFromFile);
