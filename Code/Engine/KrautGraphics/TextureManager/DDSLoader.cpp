...
namespace AE_NS_GRAPHICS
{
    ...
    if (!File.Open (szFile))
		...
		if (Surf.ddpfPixelFormat.dwFlags & DDPF_FOURCC)
		{
			...
			if (aeStringFunctions::CompareEqual (szFourCC, "DXT3"))
				m_Compression = AE_COMPR_DXT3;
			else
			if (aeStringFunctions::CompareEqual (szFourCC, "DXT5"))
				m_Compression = AE_COMPR_DXT5;
			else
			if (aeStringFunctions::CompareEqual (szFourCC, "q"))
				m_Compression = AE_COMPR_RGBA16F;
			else
			if (aeStringFunctions::CompareEqual (szFourCC, "t"))
