...
namespace AE_NS_GRAPHICS
{
  ...
  aeShaderManImpl::aeShaderProgram* aeShaderManImpl::m_pCurrentProgram = nullptr;
#endif
  ...
  void aeShaderManager::setImmediateModeShader(void)
  {
    ...
    if (g_RenderAPI == AE_RA_OPENGL)
    {
      ...
      aeShaderManImpl::m_pCurrentProgram = nullptr;
      glUseProgram(0);
    }
#endif
  }
  ...
  AE_GPU_VENDOR::Enum aeShaderManager::getGPUVendor(void)
  {
    if (aeShaderManImpl::m_GPUVendor == AE_GPU_VENDOR::AE_VENDOR_UNCHECKED)
    {
      ...
      if (g_RenderAPI == AE_RA_OPENGL)
      {
        ...
        else if (aeStringFunctions::FindFirstStringPos(s.c_str(), "ATI ") != -1)
        {
