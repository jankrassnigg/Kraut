...
namespace AE_NS_GRAPHICS
{
	...
	int LUAFUNC_Const (lua_State* state)
	{
		...
		g_sCodeBuilt += getVariableReplacement ((AE_VAR_TYPES) uiVar) + " " + sVar 
				+ " = " + getVariableReplacement ((AE_VAR_TYPES) uiVar) + " (" + sVal + ")";
	...
	int LUAFUNC_Sample (lua_State* state)
	{
		...
		if (g_RenderAPI == AE_RA_OPENGL)
		{
			...
			if (g_ShaderType == AE_ST_GEOMETRY)
				sType = "_GS";
		...
		//if (!g_Varyings.empty ())
		{
			...
			if (g_RenderAPI == AE_RA_OPENGL)
			{
				for (aeUInt32 ui = 0; ui < g_Varyings.size (); ++ui)
				{
					...
					if (g_Varyings[ui].m_Modifier == AE_VM_FLAT)
		...
		// input
		{
			...
			if (g_RenderAPI == AE_RA_OPENGL)
			{
				for (aeUInt32 ui = 0; ui < g_VaryingsIn.size (); ++ui)
				{
					...
					if (g_VaryingsIn[ui].m_Modifier == AE_VM_FLAT)
		...
		// output
		{
			...
			if (g_RenderAPI == AE_RA_OPENGL)
			{
				for (aeUInt32 ui = 0; ui < g_VaryingsOut.size (); ++ui)
				{
					...
					if (g_VaryingsOut[ui].m_Modifier == AE_VM_FLAT)
