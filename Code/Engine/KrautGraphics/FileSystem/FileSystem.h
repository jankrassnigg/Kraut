...
#include <KrautGraphics/FileSystem/DataDirectory.h>
...
namespace AE_NS_FOUNDATION
{
  ...
  class AE_GRAPHICS_DLL aeFileSystem
  {
  public:
    ...
    //! Opens a file for writing. Returns a directory-writer or nullptr. Usually not to be called directly, but by some file-abstraction.
    static aeDataDirectoryFileWriter* OpenFileToWrite(const char* szFile /*, bool bReplacingTokens = false*/);
    //! Opens a file for reading. Returns a directory-reader or nullptr. Usually not to be called directly, but by some file-abstraction.
    ...
    //! Sets the hook (callback) to be called whenever a file of a certain type shall be opened. Pass nullptr to remove a hook.
    ...
    static bool MakeValidPath(const char* szPath, bool bForWriting, aeString* out_sAbsolutePath, aeString* out_sDataDirRelativePath);

    //void EnumFilesInDirectory (const char* szDirectory, bool bRecursive, CALLBACK cb, void* pPassThrough = nullptr);
  ...
  private:
    ...
    struct aeDataDir
    {
      ...
      aeString m_sDataDirectoryGroup;
    ...
    static aeMap<aeString, AE_FILE_TYPE_READER_HOOK> m_FileTypeHooks;
