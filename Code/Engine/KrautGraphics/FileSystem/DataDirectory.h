...
namespace AE_NS_FOUNDATION
{
  ...
  class AE_GRAPHICS_DLL aeDataDirectoryFileReader
  {
  public:
    //! Constructor, takes the directory-handler and file-name.
    aeDataDirectoryFileReader(aeDataDirectoryHandler* pHandler, const char* szFile)
      : m_pDirectoryHandler(pHandler)
      , m_sFilePath(szFile)
    {
    }
    ...
    const aeString& GetFilePath(void) const { return (m_sFilePath); }
  ...
  class AE_GRAPHICS_DLL aeDataDirectoryFileWriter
  {
  public:
    //! Constructor, takes the directory-handler and file-name.
    aeDataDirectoryFileWriter(aeDataDirectoryHandler* pHandler, const char* szFile)
      : m_pDirectoryHandler(pHandler)
      , m_sFilePath(szFile)
    {
    }
  ...
  class AE_GRAPHICS_DLL aeDataDirectoryHandler
  {
  public:
    //! Constructor. Takes the path to the directory to manage.
    aeDataDirectoryHandler(const char* szDirectory)
    ...
    //! \todo Implement this
    //virtual void EnumFilesInDirectory (const char* szDirectory, bool bRecursive, CALLBACK, void* pPassThrough = nullptr) = 0;

    //! Opens the given file for reading. Returns a directory-reader, if successful. nullptr otherwise.
    virtual aeDataDirectoryFileReader* OpenFileToRead(const char* szFile) = 0;
    //! Opens the given file for writing. Returns a directory-writer, if successful. nullptr otherwise.
    virtual aeDataDirectoryFileWriter* OpenFileToWrite(const char* szFile) = 0;

    //! Returns the directory that this handler manages.
    const aeString& GetDataDirectory(void) const { return (m_sDirectory); }
