...
namespace Kraut
{
    ...
    if (m_pTreeStructureDesc->m_bGrowProceduralTrunks)
    {
      ...
      for (aeUInt32 trunks = 0; trunks < 3; ++trunks)
      {
        ...
        for (aeUInt32 t = 0; t < uiTrunks; ++t)
        {
          ...
          {
            const float fRotationalDev = (spawnDesc.m_fMaxRotationalDeviation > 0) ? (m_RNG.Rand(spawnDesc.m_fMaxRotationalDeviation * 2) - spawnDesc.m_fMaxRotationalDeviation) : m_RNG.Rand(1);
  ...
  Kraut::BranchStats TreeStructureGenerator::CreateBranchDesc(const Kraut::SpawnNodeDesc& spawnDesc, Kraut::BranchRandomData& rd, float fMaxAbsRotationalDeviation /*= 180.0f*/)
  {
    Kraut::BranchStats d;
    d.m_RandomData = rd;
