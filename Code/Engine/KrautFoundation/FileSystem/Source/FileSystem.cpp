...
namespace AE_NS_FOUNDATION
{
  ...
  aeMap<aeFileExtension, AE_FILE_TYPE_READER_HOOK> aeFileSystem::m_FileTypeHooks;
...
restart:
  ...
  {
    const aeFilePath sFile = aePathFunctions::MakeCleanPath (szFile);
    szFile = sFile.c_str ();

    aeFileEvent fe;
    fe.m_szFileOrDirectory = szFile;

    //aeDataDirectoryFileWriter* pResult = NULL;
    ...
    for (aeUInt32 uiSearchDir = 0; uiSearchDir < SearchDirs.size (); ++uiSearchDir)
    {
      for (aeInt32 i = m_DataDirectories.size () - 1; i >= 0; --i)
      {
        ...
        aeFilePath sDataDir = m_DataDirectories[i].m_pHandler->GetDataDirectory ();
  ...
  aeDataDirectoryFileReader* aeFileSystem::OpenFileToRead (const char* szFile, bool bAllowReaderHooks/*, bool bReplacingTokens*/)
  {
    ...
    if (bAllowReaderHooks)
    {
      ...
      aeFileExtension sExt = aePathFunctions::GetFileExtension (szFile);
      sExt.ToUpperCase ();

      // check if there is a hook for it
      aeMap<aeFileExtension, AE_FILE_TYPE_READER_HOOK>::iterator it = m_FileTypeHooks.find (sExt);
    ...
    //aeDataDirectoryFileReader* pResult = NULL;
    ...
    for (aeUInt32 uiSearchDir = 0; uiSearchDir < SearchDirs.size (); ++uiSearchDir)
    {
      for (aeInt32 i = m_DataDirectories.size () - 1; i >= 0; --i)
      {
        ...
        if (pReader != NULL)
        {
          const aeFilePath sPath = pReader->GetFilePath ();
  ...
  {
    aeFileExtension sExt = szFileExtension;
    sExt.ToUpperCase ();
    if (sExt.StartsWith ("."))
      sExt = &(sExt.c_str ()[1]);
  ...
  {
    //if (ExecuteActionOnPath (szFile, Action_DeleteFile, NULL))
      //return;
  ...
  //{
  //	aeFilePath sFilePath = szPathWithTokens;
  ...
  //	aeFilePath sToken = &(sFilePath.c_str ()[iTokenPos + 1]);
  ...
  */
  ...
  {
    const aeFilePath sFolderStructure = aePathFunctions::MakeCleanPath (szPathToFileOrFolder);
    
    aeFilePath sPath;
    aeFileName sSubFolder;
  ...
  {
    //if (ExecuteActionOnPath (szFile, Action_CreateFolderStructure, NULL))
      //return (true);
  ...
  bool aeFileSystem::MakeValidPath (const char* szPath, bool bForWriting, aeFilePath* out_sAbsolutePath, aeFilePath* out_sDataDirRelativePath)
  {
    aeFilePath sPath = aePathFunctions::MakeCleanPath (szPath);
    bool bRet = false;
    ...
    if (bWrite)
    {
      const aeFilePath sFileNameAndExt = aePathFunctions::GetFileNameAndExtension (sPath);
