...
namespace AE_NS_FOUNDATION
{
  ...
  */
  ...
  {
    //! Returns true if the given string is a NULL pointer or an empty string ("").
	...
	/*! Returns the number of charachters that would have been required to output the entire string.\n
		szDst may be NULL.\n
	...
	*/
    ...
    //! Returns how many characters are required to store the formatted string, including the \0-terminator.
    static aeUInt32 GetRequiredSizeForFormat (const char* szFormat, ...);
    //! Returns how many characters are required to store the formatted string, including the \0-terminator.
    static aeUInt32 GetRequiredSizeForFormat (const char* szFormat, va_list ap);
