...
namespace AE_NS_FOUNDATION
{
  ...
  {
    AE_CHECK_DEV (szDst != NULL, "aeStringFunctions::FormatArgs: Cannot work on a NULL-String.");
    AE_CHECK_DEV (szFormat != NULL, "aeStringFunctions::FormatArgs: Cannot work with a NULL-String as Format.");

    return aeStringFunctions::vsnprintf (szDst, uiDstSize, szFormat, ap);

    //// makes sure ALL formats are in "C" locale
    //aeLocaleFixer lx;

    //const aeUInt32 uiTempBufferSize = 1024 * 1024;
    //char* pData = (char*) aeMemory::AccquireTempBuffer (uiTempBufferSize);
    //vsnprintf (pData, uiTempBufferSize - 1, szFormat, ap);

    //const aeUInt32 uiLength = aeMath::Min (uiDstSize-1, GetLength (pData));
    //pData[uiLength] = '\0';

    //Copy (szDst, uiDstSize, pData, uiLength);

    //aeMemory::ReleaseTempBuffer (pData);

    //return (uiLength);
  }

  aeUInt32 aeStringFunctions::GetRequiredSizeForFormat (const char* szFormat, ...)
  {
    va_list ap;
    va_start (ap, szFormat);

    const aeUInt32 uiRes = GetRequiredSizeForFormat (szFormat, ap);
  ...
  aeUInt32 aeStringFunctions::GetRequiredSizeForFormat (const char* szFormat, va_list ap)
  {
    AE_CHECK_DEV (szFormat != NULL, "aeStringFunctions::GetRequiredSizeForFormat: Cannot work with a NULL-String as Format.");
    ...
    // vsprintf_s is totally useless
    vsnprintf (pData, uiTempBufferSize - 1, szFormat, ap);

    aeMemory::ReleaseTempBuffer (pData);

    return (GetLength (pData) + 1);
  }
